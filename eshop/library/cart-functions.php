...
function addToCart()
{
  global $etomite;//added Cris D
	// make sure the product id exist
	if (isset($_GET['p']) && (int)$_GET['p'] > 0) {
		$productId = (int)$_GET['p'];
	} else {
		$redirectURL=$etomite->makeURL($etomite->documentIdentifier,'','');
		//header('Location: index.php?id=69');
	}
	...
	if (dbNumRows($result) != 1) {
		...
		$redirectURL=$etomite->makeURL($etomite->documentIdentifier,'','?nav=cart');
			return $redirectURL;
...
function updateCart()
{
 global $etomite;
	...
	$redirectURL=$etomite->makeURL($etomite->documetIdentifier,'','');
	
	/*header("Location: $returnUrl" . $_SESSION['shop_return_url']);*/
	
	
	} else {
		$redirectURL=$etomite->makeURL($etomite->documentIdentifier,'','?nav=cart');//link to cart
		//header('Location: cart.php');	
	}
	return $redirectURL;//return redirect nav URL
	exit;
}
