...
{
 global $shopConfig;//added shopConfig as a global variable to allow other functions to use it- Cris D.
...
//format the style of your currency using the php function number_format()
function displayAmount($amount)
{
	global $shopConfig;
	return $shopConfig['currency'] . number_format($amount, 2, '.', ',');// 1,234.67
}
...
function displayError()
{
	if (isset($_SESSION['plaincart_error']) && count($_SESSION['plaincart_error'])) {
		...
		$out= '<table id="errorMessage" width="550" align="center" cellpadding="20" cellspacing="0"><tr><td>';
		for ($i = 0; $i < $numError; $i++) {
			$out .= '&#8226; ' . $_SESSION['plaincart_error'][$i] . "<br>\r\n";
		}
		$out .= '</td></tr></table>';
...
function getPagingLink($sql, $itemPerPage = 10, $strGet = '')
{
	...
	if ($totalPages > 1) {
		...
		if ($pageNumber > 1) {
			$page = $pageNumber - 1;
			if ($page > 1) {
				$prev = " <a href=\"".$etomite->makeURL($etomite->documentIdentifier,'',"?page=".$page."&".$strGet."/")."\">[Prev]</a> ";
			} else {
				$prev = " <a href=\"".$etomite->makeURL($etomite->documentIdentifier,'',"?page=".$page."&".$strGet)."\">[Prev]</a> ";
			}	
				
			$first = " <a href=\"".$etomite->makeURL($etomite->documentIdentifier,'',"?page=".$page."&".$strGet)."\">[First]</a> ";
		} else {
		...
		if ($pageNumber < $totalPages) {
			$page = $pageNumber + 1;
			$next = " <a href=\"".$etomite->makeURL($etomite->documentIdentifier,'',"?page=".$page."&".$strGet)."\">[Next]</a> ";
			$last = " <a href=\"".$etomite->makeURL($etomite->documentIdentifier,'',"?page=".$totalPages."&".$strGet)."\">[Last]</a> ";
		} else {
		...
		for($page = $start; $page <= $end; $page++)	{
			if ($page == $pageNumber) {
				...
				if ($page == 1) {
					$pagingLink[] = " <a href=\"".$etomite->makeURL($etomite->documentIdentifier,'',"?page=".$page."&".$strGet)."\">$page</a> ";
				} else {	
					$pagingLink[] = " <a href=\"".$etomite->makeURL($etomite->documentIdentifier,'',"?page=".$page."&".$strGet)."\">$page</a> ";
				}	
			}
